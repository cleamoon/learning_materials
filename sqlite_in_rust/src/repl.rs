...
impl Hinter for REPLHelper {
    ...
    // Takes the currently edited line with the cursor position and returns the string that should
    // be displayed or None if no hint is available for the text the user currently typed
...
impl Validator for REPLHelper {
    ...
    // Configure whether validation is performed while typing or only when user presses Enter
    fn validate_while_typing(&self) -> bool {
        self.validator.validate_while_typing()
    }
}

// Implementing syntax highligher with ANSI color
impl Highlighter for REPLHelper {
    ...
    // Takes the hint and returns the highlighted version
    fn highlight_hint<'h>(&self, hint: &'h str) -> Cow<'h, str> {
        Owned("\x1b[1m".to_owned() + hint + "\x1b[m")
    }

    // Takes the currently edited line with the cursor position and returns the highlighted version
    fn highlight<'l>(&self, line: &'l str, pos: usize) -> Cow<'l, str> {
        self.highlighter.highlight(line, pos)
    }

    // Tells if line needs to be highlighted  when a specific char is typed or when cursor is moved under
    // a specific char. Used to optimize refresh when a character is inserted or the cursor is moved
